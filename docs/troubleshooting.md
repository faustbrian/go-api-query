...
## Local `go-validation` replacement

Before the dependency's implementation commit is published, this repository
uses `replace github.com/faustbrian/go-validation => ../go-validation` for fully
offline development. Release order is: publish `go-validation`, pin its public
